  ...
  productApi: {
    sourceImport: {
      importSource: async (request) => {
        const data = await readSourceImportResponse(await fetch("/api/source-import", {
          method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(request),
        }));
        return { content: typeof data.content === "string" ? data.content : "", headers: data.headers || {}, parseResult: data.parseResult };
      },
    },
    templates: { catalogEnabled: false, builtinEngagementEnabled: false },
    rules: createRulesProductApi(),
  },
  loadSubscription: (id) => fetch(`/api/subscriptions/${encodeURIComponent(id)}`, { cache: "no-store" }),
  subscription: {
    loginHref: "/login", autoUpdateIntervalPolicy: LOCAL_AUTO_UPDATE_POLICY,
    saveSubscription: ({ isEditing, subscriptionId, payload }) => fetch(
      isEditing && subscriptionId ? `/api/subscriptions/${encodeURIComponent(subscriptionId)}` : "/api/subscriptions",
      { method: isEditing ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) }
    ),
  },
};

export default function EditorPage() { return <HomeSurface adapter={localHomeAdapter} />; }
